...
import { getQuickFeatures, createQuickFeature, updateQuickFeature } from "@/app/admin/actions"
import type { QuickFeature } from "@/lib/db"

export function QuickFeaturesForm() {
  const [isLoading, setIsLoading] = useState(true)
  const [features, setFeatures] = useState<QuickFeature[]>([])
  ...
  useEffect(() => {
    async function loadFeatures() {
      try {
        const data = await getQuickFeatures()
        setFeatures(data)
      } catch (error) {
        console.error("Erro ao carregar recursos rápidos:", error)
        toast({
          title: "Erro",
          description: "Não foi possível carregar os recursos rápidos.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  function handleChange(id: number, field: keyof QuickFeature, value: any) {
    ...
    try {
      ...
      for (const feature of features) {
        if (feature.id < 0) {
          ...
          await createQuickFeature(featureData)
        } else {
          // Recurso existente
          await updateQuickFeature(feature.id, feature)
        }
      }

      // Recarregar os recursos após salvar
      const updatedFeatures = await getQuickFeatures()
      setFeatures(updatedFeatures)

      toast({
        title: "Recursos salvos com sucesso!",
        description: "Os recursos rápidos foram atualizados na página inicial.",
      })
    } catch (error) {
      console.error("Erro ao salvar recursos:", error)
      toast({
        title: "Erro ao salvar",
        description: "Ocorreu um erro ao salvar os recursos rápidos.",
        variant: "destructive",
      })
    } finally {
